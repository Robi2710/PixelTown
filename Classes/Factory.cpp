...
Factory::Factory(const std::string &type, int capacity, int costMoney,
                 int costMaterials, const std::string &product,
                 int productionRate, int materialGenerationRate,
                 int GenerationInterval)
    : Buildings(type, capacity, costMoney, costMaterials), product(product),
...
void Factory::updateBuilding(Resources &resources) {
  ...
  auto timeDiff = std::chrono::duration_cast<std::chrono::seconds>(
      currentTime - lastProductionTime);
  ...
  if (timeDiff.count() >= generationInterval) {
    ...
    std::cout << "Factory generated " << materialGenerationRate
              << " materials\n";
  }
}
...
/*void Factory::showProductionInfo() const {
    std::cout << "Factory Type: " << getType() << ", Producing: " << product
              << ", Production Rate: " << productionRate << " units/month" <<
std::endl;
}*/
...
/*void Factory::generateMaterials(Resources& resources) {
    auto currentTime = std::chrono::system_clock::now();
    auto timeDiff = std::chrono::duration_cast<std::chrono::seconds>(currentTime
- lastProductionTime);

    if (timeDiff.count() >= generationInterval) {
        resources.setMaterials(resources.getMaterials() +
materialGenerationRate); lastProductionTime = currentTime;
    }
}*/